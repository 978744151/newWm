...
Page({
    data: {
      ...
      scrollHeightM: 880,
        ...
        duration1: 300,
        currentIndex: 0,
        idgroup: {iid: null,cateid: null},
        skuAttrname: '',//选择的规格与属性名称拼接
        skuId: '',//当前选择的规格的id
		isScene:false,   //微信扫码进入
		sceneData:null,
        storeid:null,
        tablesid:null
    ...
    onLoad: function (event) {    
        ...
        this.setData({access_token,dir,event,totalprice: event.totalprice || '0'});
        shop_id = wx.getStorageSync('shop_id');


        this.setData({
            storeid: event.storeid || '',
            tablesid: event.tablesid || ''
        })
    ...
    onShows: function (e) {
        ...
        if (app.scene == '1011') {  //微信扫描二维码进入
            this.setData({
                isScene: true
            })
            let that = this;
            wx.request({
                url: app._host + '/index.php?ctrl=wxapp&action=shopTableInfo&id=' + that.data.storeid + '&tablesid=' + that.data.tablesid,
        ...
        wx.setNavigationBarTitle({title: '堂食'});
        this.setData({upgrade: wx.getStorageSync('upgrade')});
        // 测试随机数
        var random = Math.floor(Math.random() * 10);
        if (random <= 5) {
            this.setData({ randomShow: 1 });
        } else {
            this.setData({ randomShow: 2 });
        }
        var shopTheme = wx.getStorageSync('shopTheme');
        //根据接口返回重新赋值模板颜色
        this.setData({ crFlag: shopTheme})
        ...
        if (this.data.color == '#ffffff') {
            wx.setNavigationBarColor({frontColor: '#000000',backgroundColor: this.data.color});
            wx.setStorage({key: 'color',data: '#000000'})
        } else {
            wx.setNavigationBarColor({frontColor: '#ffffff',backgroundColor: this.data.color})
            wx.setStorage({key: 'color',data: this.data.color})
        }
        wx.setStorage({ key: 'crFlag',data: this.data.crFlag});
        this.activity();
        API.IndexData(this, this.data.event, this.data.dir,function(that){
          console.log(that.data.goodsType)
          var _isResh = app.isResh;
          var _cartData = wx.getStorageSync('cartTemp_ts') || {};
          ...
          if (app.isList == 1) {
            ...
            if (_isResh == 0) {
              if (utils.isEmpty(_cartData)) {
                ...
                that.setData({cartData: _cartData,goodsType: _goodsType})
              }
            }
          }
          try {
            ...
            var res = wx.getSystemInfoSync();
              var scrollHeight = res.windowHeight - that.countPX(96) - that.countPX(68) - that.countPX(50)
    ...
    specChoose: function (e) {
        this.setData({ currentIndex: e.currentTarget.dataset.parentid });
    ...
    attrChoose: function (e) {
        ...
        console.log(tagsId)
        that.setData({ skuAttr: skuAttr, tagsId });
    },
    // 选规格事件--弹出规格框
    showSkuHandle: function (e) {
        ...
        this.setData({
            ...
            canhefei: canhefei,
            skuprice: skuprice,
            idgroup: { iid: e.currentTarget.dataset.iid, cateid: e.currentTarget.dataset.cateid },
            skuAttr: curfood,
            skuShow: true
        });
        // 关闭图片放大的弹窗
    ...
    selectOk: function () {
        ...
        if (skuAttr.tags.length > 0) {
            ...
            if (skuAttrname == '') { skuAttrname = skuAttr.types[0].name }
            skuAttrname;
        }
        this.setData({ moneyShow: true, skuShow: false })
        ...
        try {
            wx.setStorageSync('cartTemp_ts', goodsCart);
    ...
    activity: function () {
        var that = this;
        zynetwork.apiUrl('activity/queryValidActivity', { shop_id: app.shopId }, (res) => {
            if (res.status <= 0) {
                var actNum = 0;
                var data = res.data;
                ...
                that.setData({ hasReduction: hasReduction,actiStatus: res.status, actNum: actNum, vouchers: data.vouchers, reduction: data.reduction, acData: data })
            }
        });
    },
    shopDet: function () {
    ...
    intoShop: function () {
        var that = this;
        zynetwork.apiUrl('activity/queryIsHasIntoVouchers', { shop_id: app.shopId }, (res) => {
            if (res.status <= 0) {
                that.setData({
                    isStatus: true,
                    status: 100,
                ...
                that.setData({ status: res.status, intoCoupon: res.data })
            }
        });
    },
    //进店就送去领取按钮
    accept: function () {
        var that = this;
        zynetwork.apiUrl('activity/givingVouchers', { shop_id: app.shopId, get_rule: 'into' }, (res) => {
            if (res.status <= 0) {
                wx.setStorage({
                    key: "data",
                    data: res.data
    ...
    goCoupons: function (e) {
        ...
        zynetwork.apiUrl('activity/givingVouchers', { shop_id: app.shopId }, (res) => {
            if (res.status <= 0) {
                that.setData({ isStatus: false })
                wx.setStorage({
                    key: "data",
                    data: res.data
                });
    ...
    close: function () {
        this.setData({ isStatus: true, status: 100 });
    },
    actiFuc: function () {
        this.setData({ jShow: !this.data.jShow, actiShow: !this.data.actiShow })
        ...
        wx.showModal({
            ...
            success: function (res) {
                if (res.confirm) {
                    ...
                    that.setData({ selectedCate: that.data.selectedCate })
                    ...
                    try {
                        wx.setStorageSync('cartTemp_ts', cartData);
    ...
    tapPay: function (e) {
      pickDishes.tapPay(this,'createOrder_ts')
      return;
      if(this.data.shopInfo.is_open=='0'){
        ...
        var totalPrice = Number(this.data.cartData.totalPrice);
        if (this.data.cartData.totalPrice < 0 || !(this.data.cartData.foodsNum > 0)) {
            return;
        } else {
           
            totalPrice = Number(totalPrice - Number(this.data.cartData.totalBag) + Number(this.data.cartData.totalBag)).toFixed(2);
            zynetwork.apiUrl('activity/getValidVouchersByPayMoney', { pay_money: totalPrice }, (res) => {
                if (res.status <= 0) {
                    wx.setStorage({
                        key: "usableData",
                        data: res.data
				...
				let url = null;
				if (that.data.isScene){
					url = '/pages/createOrder_ts/index?youhui=' + youhui + '&reduction_id=' + reduction_id + '&skuids=' + skuids + '&hasReduction=' + hasReduction + '&tableid=' + that.data.sceneData.info.title
				}else{
					url = '/pages/createOrder_ts/index?youhui=' + youhui + '&reduction_id=' + reduction_id + '&skuids=' + skuids + '&hasReduction=' + hasReduction
    ...
    anewCart: function () { //再来一单重新添加本地缓存
      ...
      this.setData({
        ...
        wx.setStorageSync('cartTemp_ts', _cartTemp);
    },
    ...
    cartAdd: function (e) {
      ...
      app.globalData.lastTapTime = time;
        ...
        this.setData({ cartData: this.data.cartData, goodsType: goodsType });
        wx.setStorageSync('cartTemp_ts', this.data.cartData);
        wx.setStorageSync('goodsType', goodsType);
   ...
   */
    addcart(tPrice, that) {
        let totalPrice = Number(Number(tPrice) - Number(that.data.cartData.totalBag)).toFixed(2);
        if (that.data.actiStatus <= 0 && that.data.reduction != '') {
            ...
            var discountTip = '', full_money = 0, reduction_money = 0;
            if (isM == -1) {
 ...
 */
  ...
  setTimeout(function () {
    ...
    /**
     * [numChange  精准到两位小数点-------主要针对购物车 显示那块]
    ...
    numChange(that){
        that.setData({
            ...
            tsTotalPrice: Number(Number(that.data.cartData.totalPrice) - Number(that.data.cartData.totalBag) + Number(that.data.cartData.totalBag)).toFixed(2),
